...
struct DownloadNewModel: Codable, Equatable, Identifiable {
    ...
    let shotsNoticia: DownloadShotNoticias?
    
    init(pId: String, 
         pTypenameNoticia: String,
         pTitleNoticia: String,
         pSubtitleNoticia: String,
         pLeadingNoticia: String,
         pUrlNoticia: String, pBodyNoticia: String, 
         pShotsNoticia: DownloadShotNoticias) {
        
        self.id = pId
        self.typenameNoticia = pTypenameNoticia
        self.titleNoticia = pTitleNoticia
        self.subtitleNoticia = pSubtitleNoticia
        self.leadingNoticia = pLeadingNoticia
        self.urlNoticia = pUrlNoticia
        self.bodyNoticia = pBodyNoticia
        self.shotsNoticia = pShotsNoticia
    }
    static func == (lhs: DownloadNewModel, rhs: DownloadNewModel) -> Bool {
        return lhs.titleNoticia == rhs.titleNoticia
    }
}

struct DownloadShotNoticias: Codable {
    let identificadorUno: String?
    
    init(pIdentificadorUno: String) {
        self.identificadorUno = pIdentificadorUno
    }
}
