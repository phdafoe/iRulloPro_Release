...
// Input
protocol PortadaTenisProviderInputProtocol: BaseProviderInputProtocol {
    func fecthDataPortadaTenis()
}


final class PortadaTenisProvider: BaseProvider {
    
    weak var viewModel: PortadaTenisPresenterProtocol? {
        super.baseViewModel as? PortadaTenisPresenterProtocol
    }
    let networkService: Requestable = NetworkRequestable()
    
    
    func callBackPortadasTenis(dictionary: [[String: Any]]?) -> [PortadaTenisModel]? {
        let nuevoArray = dictionary?.dropFirst(2)
        let arrayPortadaFutbol: [PortadaTenisModel]? = nuevoArray?.compactMap {
            PortadaTenisModel(name: $0["name"] as? String,
...
extension PortadaTenisProvider: PortadaTenisProviderInputProtocol {
    
    func fecthDataPortadaTenis() {
        
        self.networkService.request(RequestModel(service: PortadaTenisProviderService.portadaTenis)) { myTenisDictionary, error in
            if let errorUnw = error  {
                print(errorUnw)
                self.viewModel?.setPortadaTenis(completion: .failure(errorUnw))
            } else {
                DispatchQueue.main.async {
                    self.viewModel?.setPortadaTenis(completion: .success(self.callBackPortadasTenis(dictionary: myTenisDictionary?["areas"] as? [[String: Any]])))
                }
            }
        }
...
enum PortadaTenisProviderService {
    case portadaTenis
}

extension PortadaTenisProviderService: Service {
    var baseURL: String {
        return Helpers.customUrl().apiHost
    }
    
    var path: String {
        return Helpers.customUrl().portadasTenis
    }
