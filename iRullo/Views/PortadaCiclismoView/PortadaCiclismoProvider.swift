...
// Input
protocol PortadaCiclismoProviderInputProtocol: BaseProviderInputProtocol {
    func fecthDataPortadaCiclismo()
}


final class PortadaCiclismoProvider: BaseProvider {
    
    weak var viewModel: PortadaCiclismoPresenterProtocol? {
        super.baseViewModel as? PortadaCiclismoPresenterProtocol
    }
    let networkService: Requestable = NetworkRequestable()
    
    
    func callBackPortadasCiclismo(dictionary: [[String: Any]]?) -> [PortadaCiclismoModel]? {
        let nuevoArray = dictionary?.dropFirst(2)
        let arrayPortadaFutbol: [PortadaCiclismoModel]? = nuevoArray?.compactMap {
            PortadaCiclismoModel(name: $0["name"] as? String,
...
extension PortadaCiclismoProvider: PortadaCiclismoProviderInputProtocol {
    
    func fecthDataPortadaCiclismo() {
        
        self.networkService.request(RequestModel(service: PortadaCiclismoProviderService.portadaCiclismo)) { myCiclismoDictionary, error in
            if let errorUnw = error  {
                print(errorUnw)
                self.viewModel?.setPortadaCiclismo(completion: .failure(errorUnw))
            } else {
                DispatchQueue.main.async {
                    self.viewModel?.setPortadaCiclismo(completion: .success(self.callBackPortadasCiclismo(dictionary: myCiclismoDictionary?["areas"] as? [[String: Any]])))
                }
            }
        }
...
enum PortadaCiclismoProviderService {
    case portadaCiclismo
}

extension PortadaCiclismoProviderService: Service {
    var baseURL: String {
        return Helpers.customUrl().apiHost
    }
    
    var path: String {
        return Helpers.customUrl().portadasCiclismo
    }
