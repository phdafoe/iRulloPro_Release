//
//  PortadaMasDeporteProvider.swift
//  iRullo
//
...
// Input
protocol PortadaMasDeporteProviderInputProtocol: BaseProviderInputProtocol {
    func fecthDataPortadaMasDeporte()
}

final class PortadaMasDeporteProvider: BaseProvider {
    
    
    weak var viewModel: PortadaMasDeportePresenterProtocol? {
        super.baseViewModel as? PortadaMasDeportePresenterProtocol
    }
    let networkService: Requestable = NetworkRequestable()
    
    
    func callBackPortadasMasDeporte(dictionary: [[String: Any]]?) -> [PortadaMasDeporteModel]? {
        let nuevoArray = dictionary?.dropFirst(2)
        let arrayPortadaFutbol: [PortadaMasDeporteModel]? = nuevoArray?.compactMap {
            PortadaMasDeporteModel(name: $0["name"] as? String,
...
extension PortadaMasDeporteProvider: PortadaMasDeporteProviderInputProtocol {
    
    func fecthDataPortadaMasDeporte() {
        
        self.networkService.request(RequestModel(service: PortadaMasDeporteProviderService.portadaMasDeporte)) { myMasDeporteDictionary, error in
            if let errorUnw = error  {
                print(errorUnw)
                self.viewModel?.setPortadaMasDeporte(completion: .failure(errorUnw))
            } else {
                DispatchQueue.main.async {
                    self.viewModel?.setPortadaMasDeporte(completion: .success(self.callBackPortadasMasDeporte(dictionary: myMasDeporteDictionary?["areas"] as? [[String: Any]])))
                }
            }
        }
...
enum PortadaMasDeporteProviderService {
    case portadaMasDeporte
}

extension PortadaMasDeporteProviderService: Service {
    var baseURL: String {
        return Helpers.customUrl().apiHost
    }
    
    var path: String {
        return Helpers.customUrl().portadasMasDeporte
    }
