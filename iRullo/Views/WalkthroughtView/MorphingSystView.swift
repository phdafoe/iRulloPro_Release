...
struct MorphingSystView: View {
    
    var symbol: String
    var config: Config
    
    @State private var trigger: Bool = false
    @State private var displayingSymbols: String = ""
    ...
    var body: some View {
        ...
        .task {
            guard displayingSymbols == "" else { return }
            displayingSymbols = symbol
        }
    }
    ...
    func ImageView() -> some View {
        ...
        KeyframeAnimator(initialValue: CGFloat.zero, trigger: trigger) { radius in
            Image(systemName: displayingSymbols)
                .font(config.font)
                .blur(radius: radius)
                .frame(width: config.frame.width, height: config.frame.height)
...
#Preview {
    MorphingSystView(symbol: "gearshape.fill", config: .init(font: .system(size: 100, weight: .bold),
